class MenuBurger {
    ...
    static fillFilters(data, personId) {
        $('.person-id-input').val(personId)
    ...
    static openHistory() {
        ...
        historyElement.on('click', function () {
            ...
            MenuBurger.fillFilters(currentData[0], personIdValue)

            const graph = new Graph({
                personId: currentData[0].personId,
                data: {
                    node: currentData[0].nodes,
            ...
            MenuBurger.closeHistory()
        })
    }

    static openFavourites() {
        this.closeHistory()
    ...
    static async openAdminPanel() {
        await API.checkAccessToAdminPanel()
    }

    static exit() {
        LocalStorage.remove('login')
        LocalStorage.remove('password')
        if (LocalStorage.get('email') != null) LocalStorage.remove('email')
...
$(document).ready(function (event) {
    $('.burger').on('click', MenuBurger.showMenu)

    $('.save-to-favourites-btn').attr('disabled', 'disabled').on('click', LocalStorage.saveGraphToFavourites)

    $('.favourites-btn').on('click', MenuBurger.openFavourites)
    $('.history-btn').on('click', MenuBurger.openHistory)
    $('.admin-btn').on('click', MenuBurger.openAdminPanel)
    $('.exit-btn').on('click', MenuBurger.exit)

    $('.close-history-btn').on('click', MenuBurger.closeHistory)
    $('.close-favourites-btn').on('click', MenuBurger.closeFavourites)
})
